...
{
    class CompressTester
    {
        public static void Test()
        {
            string folder = @"C:\Users\xluo\Desktop\GzipTest";
//            FileInfo[] info = { new FileInfo(Path.Combine(folder, "file2.ppt")),
//new FileInfo(Path.Combine(folder,"file3.doc")) };//获取两个文件FileInfo("file2.ppt","file3.doc")
                                                 //其中一个重载版本,压缩两个文件和一个目录("img"文件夹),保存为("myzip2.gzip")


            var directory = Directory.CreateDirectory(folder);
            var files = directory.GetFiles();
            GZip.Compress(files, new string[] {  }, folder, folder, "myzip2.gzip");
        }
    }
    public class GZip
    {
        ...
        public static GZipResult Compress(string lpSourceFolder, string searchPattern, SearchOption searchOption, string lpDestFolder, string zipFileName, bool deleteTempFile)
        {
            ...
            System.IO.FileInfo[] files = di.GetFiles("*.*", searchOption);
        ...
        /// <param name="lpDestFolder">Folder to write the zip file into</param>
        /// <param name="zipFileName">Name of the zip file to write</param>
        public static GZipResult Compress(System.IO.FileInfo[] files, string[] folders, string lpBaseFolder, string lpDestFolder, string zipFileName)
        {
            //support compress folder
            IList<System.IO.FileInfo> list = new List<System.IO.FileInfo>();
            foreach (System.IO.FileInfo li in files)
                list.Add(li);

            foreach (string str in folders)
            {
                DirectoryInfo di = new DirectoryInfo(str);
                foreach (System.IO.FileInfo info in di.GetFiles("*.*", SearchOption.AllDirectories))
                {
                    list.Add(info);
                }
            }
        ...
        /// <param name="lpDestFolder">Folder to write the zip file into</param>
        /// <param name="zipFileName">Name of the zip file to write</param>
        public static GZipResult Compress(System.IO.FileInfo[] files, string lpBaseFolder, string lpDestFolder, string zipFileName)
        {
        ...
        /// <param name="zipFileName">Name of the zip file to write</param>
        /// <param name="deleteTempFile">Boolean, true deleted the intermediate temp file, false leaves the temp file in lpDestFolder (for debugging)</param>
        public static GZipResult Compress(System.IO.FileInfo[] files, string lpBaseFolder, string lpDestFolder, string zipFileName, bool deleteTempFile)
        {
        ...
        private static void CreateTempFile(System.IO.FileInfo[] files, string lpBaseFolder, string lpTempFile, GZipResult result)
        {
            byte[] buffer;
            int count = 0;
            byte[] header;
